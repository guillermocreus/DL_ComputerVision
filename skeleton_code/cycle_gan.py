...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, device, opts):
    ...
    for iteration in range(1, opts.train_iters + 1):
        ...
        D_X_loss = ((D_X(images_X) - 1)**2).sum()
        D_X_loss /= images_X.shape[0]

        D_Y_loss = ((D_Y(images_Y) - 1)**2).sum()
        ...
        D_X_loss = (D_X(fake_X)**2).sum()
        D_X_loss /= fake_X.shape[0]

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = (D_Y(fake_Y)**2).sum()
        ...
        g_loss = ((D_X(fake_X) - 1)**2).sum()
        g_loss /= fake_X.shape[0]

        # 3. Compute the cycle consistency loss (the reconstruction loss)

        cycle_consistency_loss = (
            (images_Y - G_XtoY(G_YtoX(images_Y)))**2).sum()
        ...
        g_loss = ((D_Y(fake_Y) - 1)**2).sum()

        # 3. Compute the cycle consistency loss (the reconstruction loss)
        cycle_consistency_loss = (
            (images_X - G_YtoX(G_XtoY(images_X)))**2).sum()
